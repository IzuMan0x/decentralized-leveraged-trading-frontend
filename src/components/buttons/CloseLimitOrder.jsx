...
function CloseLimitOrder(props) {
  const [closingLimitOrder, setClosingLimitOrder] = useState(false);

  const closeLimitOrderHandler = async () => {
    console.log("Fron CloseLimitOrder", props.value, props.id);

    setClosingLimitOrder(true);
    setTimeout(() => {
      setClosingLimitOrder(false);
    ...
    const prepareMarketOrderConfig = await prepareWriteContract({
      ...
      args: [props.value, props.id],
    }).catch((err) => {
      console.log("The prepare position close error is ", err);
      setClosingLimitOrder(false);
    });
    ...
    <div>
      ...
      {closingLimitOrder ? (
        ...
        <div
          ...
          onClick={closeLimitOrderHandler}
          value={props.value}
        >
          Close Limit Order
        </div>
...
export default CloseLimitOrder;
