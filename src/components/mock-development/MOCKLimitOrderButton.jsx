...
function MOCKLimitOrderButton(props) {
  ...
  const openLimitOrder = async () => {
    ...
    const prepareLimitOrderConfig = await prepareWriteContract({
      ...
      args: [
        ...
        props.orderType,
        parseUnits?.(props.limitPrice, 8),
      ],
    }).catch((err) => {
      setOrderLoading(false);
      console.log("The prepare limitOrder is: ", err);
    });

    const limitOrderHash = await writeContract(prepareLimitOrderConfig).catch(
      (err) => setOrderLoading(false)
    );

    if (limitOrderHash != undefined) {
      setOrderLoading(false);
    }
  };
...
export default MOCKLimitOrderButton;
